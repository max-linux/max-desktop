...
class PageGtk(PageBase):
    ...
    def grub_verify_loop(self, widget, okbutton):
        if widget is not None:
            syslog.syslog("DEBUG: grub_verify_loop value=%s"%widget.child.get_text())
    ...
    def on_advanced_button_clicked(self, unused_button):
        ...
        syslog.syslog("DEBUG: on_advanced_button_clicked() grub_en=%s summary=%s"%(grub_en,summary_device))
        ...
        if summary_device is not None:
            ...
            if self.sti == "YES":
                # set /dev/sda1 as active
                self.grub_device_entry.set_active(1)
                syslog.syslog("DEBUG ==> get_active %s"%self.grub_device_entry.get_active())
                syslog.syslog("DEBUG ==> get_sensitive %s"%self.grub_device_entry.get_sensitive())
        else:
    ...
    def get_test_sti(self):
        subp = subprocess.Popen(['/usr/bin/test-sti'], stdout=subprocess.PIPE)
...
class Page(Plugin):
    def prepare(self):
        syslog.syslog("DEBUG: page:prepare()")
        self.sti=False
    ...
    def run(self, priority, question):
        syslog.syslog("DEBUG: page:run() prio=%s question=%s"%(priority, question))
        if question.endswith('/summary'):
            text = ''
            wrapper = textwrap.TextWrapper(width=76)

            if self.sti:
                text += "\n\t\t***   IMPORTANTE  ***\n\nSe ha detectado una tarjeta de backup, se instalará grub en /dev/sda1\n\n\n"
