...
if (!empty($_REQUEST["op"])) {
  switch ($_REQUEST["op"]) {
    case 'add':
      exec("sudo /usr/sbin/useradd -p ". $_REQUEST['param2'] . " -m -G adm,dialout,cdrom,floppy,audio,dip,video,plugdev,lpadmin,scanner,admin ". $_REQUEST['param1'] . "; sudo smbpasswd -a " . $_REQUEST['param1'] . "; sudo echo " . $_REQUEST['param1'] . " = " . $_REQUEST['param1'] . " >> /etc/smbusers");
      addResource($_REQUEST["param1"], $_REQUEST["param2"]);
      break;
    case 'delete':
      exec("sudo /usr/sbin/userdel " . $_REQUEST['param1']);
...
<html>
  <head>
    <title>MAXControl - Gestión de Usuarios</title>
  ...
  <body>
    ...
    <form name="recursos" action="<?php echo "http://" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']; ?>" method="post">
      <p class="xtb"><h2>Nuevo Usuario</h2>
        <br><table>
          <tr><td>Nombre:</td><td><input type="text" name="param1" size="15"></td></tr>
          <tr><td>Clave:</td><td><input type="text" name="param2" size=15"></td></tr>
          </td><td></td></tr>
        </table>
      </p><p>
